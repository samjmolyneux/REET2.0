...
def train_step(batch_data, run_info):
    ...
    #     print(f"Learning rate of parameter group {i}: {learning_rate}")
    # breakpoint()
    ####
    imgs = batch_data["img"]
    ...
    # for i in range(16):
    #     image = imgs[i].clone().detach().cpu().numpy()
    #     np = true_np[i].clone().detach().cpu().numpy()
    #     hv = true_hv[i].clone().detach().cpu().numpy()
    #     h = hv[...,0]
    #     v = hv[...,1]
        
    #     figure, axs = plt.subplots(1,4, figsize=(20,20))
    #     axs[0].imshow(cropping_center(image, (80,80)))
    #     axs[1].imshow(np)
    #     axs[2].imshow(h)
    #     axs[3].imshow(v)
    #     title = f"hovertest{i}.png"
    #     plt.savefig(title)
    ...
    orig = true_tp.clone().to("cuda").type(torch.int64)
    true_tp = torch.squeeze(true_tp).to("cuda").type(torch.int64)
    true_tp_onehot = F.one_hot(true_tp, num_classes=5)
...
def infer_step(batch_data, model):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        #Performs softmax then only takes the softmax value of the nuclear pixel.
        #So pixels above threshold 0.5 are nuclear, below 0.5 are non nuclear.
        pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)[..., 1:]

        #These are just some tests I was running.
        #pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)
        #pred_dict["np"] = torch.argmax(pred_dict["np"], dim=-1, keepdim=True)
        #pred_dict["np"] = torch.argmax(pred_dict["np"], dim=-1)
        # print(pred_dict["np"].shape)
        # breakpoint()
        if "tp" in pred_dict:
            type_map = F.softmax(pred_dict["tp"], dim=-1)
    ...
    # masks = pred_output[0].permute(2,0,1).detach().cpu().numpy()
    # import matplotlib.pyplot as plt

    # figure, axs = plt.subplots(4, 1, figsize=(50,50))
    
    # axs[0].imshow(masks[0])
    # axs[1].imshow(masks[1])
    # axs[2].imshow(masks[2])
    # axs[3].imshow(masks[3])
