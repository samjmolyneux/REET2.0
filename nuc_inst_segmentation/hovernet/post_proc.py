...
def __proc_np_hv(pred):
    ...
    #TO RECREATE THIS PART, I JUST NEED TO FIND A TORCH WAY OF REMOVE SMALL TENSORS
    blb = measurements.label(blb)[0]
    blb = remove_small_objects(blb, min_size=10) #Min_size = number of elements in object
    blb[blb > 0] = 1  # background is 0 already

    #Scale horizontal and vertical values to between 0 and 1.
    #TRY SAVING THESE AND SEE WHAT THEY LOOK LIKE
    h_dir = cv2.normalize(
...
def process(pred_map, nr_types=None, return_centroids=False):
    ...
    #Pred_inst.shape = (1000,1000) with each number being an inst
