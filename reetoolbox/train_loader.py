...
class FileLoader(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.id = 0
        self.target_gen_func = target_gen
    ...
    def __getitem__(self, idx):
        ...
        if self.shape_augs is not None:
            print("WILL NOT WORK FOR HOVERNET")
        ...
        if self.with_type:
            type_map = (ann[..., 1]).copy()
            type_map = cropping_center(type_map, self.input_shape)
        ...
        target_dict = self.target_gen_func(inst_map, self.input_shape)
        inst_map = cropping_center(inst_map, self.input_shape)
        target_dict["inst"] = inst_map
